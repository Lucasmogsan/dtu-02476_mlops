...
The starting point of the diagram is our github repo. Whenever a new pull request is made to merge into main, the workflows start, where the code is checked with ruff and unittests testing our code. Furthermore a trigger for building the the Docker files in repo is also triggered. These build Docker images contains the relevant files from the repo for the training and for the production environment. The data for training and model for prediction is however not built into the images in order to reduce the image size. These Docker images are then stored in the container registry in google cloud. Whenever we want to retrain the model, we manually start it with a make command. We do not want it to retrain everytime the is a minor change to the repo, but rebuilding the Docker image for training is ok.

The make command to initialize training launches a VM (Vitual machine) through Vertex AI based on the specifications in a configuration file and also the Docker image. Inside the container an Entrypoint script is executed, which pulls the data from a GCP (Google cloud platform) bucket, starts the training script, and pushes the saved model file to another GCP bucket. During training, the training loss and the validation accuracy is being logged in Weights & Biases. 

Then the newly trained model is evaluated by a person before pushing it to a release bucket. When the new model is push to the release bucket, the operator can then call the trigger which rebuilds the production environment and pulls the new model. The production environment consists of two containers. One for the front- and one for the backend. The backend is a FastAPI application, which provides an interface for the model. The frontend is a Streamlit application, that provides an interactive user interface, where one can upload an image of a rice grain, and the predicted result from the model will be shown.
